...
function M.get_symbol_info(bufnr, row, col)
    ...
    for _, client in ipairs(clients) do
        ...
        if not (definition and definition.result) then goto continue end

        -- Get references
        local references = client.request_sync("textDocument/references", params, 1000, bufnr)
        if not (references and references.result) then goto continue end

        local symbol_info = {
            name = M.get_symbol_name(bufnr, row, col),
            type = M.get_symbol_type(client, bufnr, definition.result[1]),
            kind = M.get_symbol_kind(definition.result[1]),
            definition = M.convert_lsp_range(definition.result[1].targetRange),
            references = vim.tbl_map(function(ref)
                return {
                    row = ref.range.start.line,
                    col = ref.range.start.character
                }
            end, references.result)
        }
...
function M.get_symbol_type(client, bufnr, definition)
    -- Try to get detailed symbol information
    local params = {
        textDocument = {
